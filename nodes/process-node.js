...
ProcessModel.Nodes.types.set("undecided", function(node) {
    node.allowedChildren = d3.set();
});

ProcessModel.Nodes.types.set("process", function(node){
	...
	if (node.edges().length === 0) {
	    ...
	    return ProcessModel.CombineEvidence(localDep, node.edges()
						.filter(function(e) {
...
ProcessModel.Nodes.types.set("issue", function(node){
    var settled = false;
